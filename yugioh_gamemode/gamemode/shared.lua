...
function PLAYER:GetDuelData()
    local duelDataJson = self:GetNWString("DuelData", "")
print("GetDuelData: DuelData JSON for player " .. self:Nick() .. ": " .. duelDataJson)
    if duelDataJson == "" then
        print("GetDuelData: DuelData is empty for player " .. self:Nick())
        return nil
    end

    local duelData = util.JSONToTable(duelDataJson)

    if not duelData.Hand then
        print("GetDuelData: Hand is missing in DuelData for player " .. self:Nick())
    end
